...
const categories = ["長期照顧中心", "護理之家", "安養中心", "日間照顧中心"];
// tag：services.name
// care: caringItem
const caringItems = [
  "鼻胃管",
...
function Searchbar() {
  ...
  const getTaiwanCity = async () => {
    const res = await axios.get(`${BASE_URL}/taiwancitys`);
  ...
  return (
    <div className="searchbar container-lg container-fluid">
      <form
        ...
        className="search-wrapper d-flex flex-md-row flex-column"
        action=""
      >
        <select
          {...register("category")}
          className="form-select fs-lg-6 fs-md-7 fs-6"
        ...
        <select
          ...
          value={cityValue}
          className="form-select fs-lg-6 fs-md-7 fs-6 d-md-block d-none"
          aria-label="Default select example"
        >
        ...
        <div className="search-wrapper-sm d-flex d-md-none">
          <select
            ...
            onChange={handleCityChange}
            value={cityValue}
            className="form-select fs-6 w-50"
            aria-label="Default select example"
          >
            <option value="">縣市</option>
            {cityData.map((item, index) => (
        ...
        <div className="d-lg-block d-grid">
          <button
            type="submit"
            className="search-primary-btn btn-width btn btn-primary-40 rounded-1 py-5 px-0 fs-lg-5 fs-md-6 fs-5 text-black d-flex justify-content-center align-items-center gap-lg-4 gap-md-0 gap-4"
          >
            <img
              src="https://raw.githubusercontent.com/Jack-Xiao-2024/Project_D01/972c057941c61b13fd6db44e084aac94405b25ed/assets/images/Icon/IconBlack/Search-B.svg"
              alt="search icon"
            />
